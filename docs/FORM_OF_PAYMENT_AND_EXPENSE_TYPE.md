...
**איך "אופי בית העסק" נקבע — לפי הלוגו ושם העסק שבו נרכש:**
המודל הוא **מודל Vision** שרואה את כל תמונת הקבלה, ולא רק טקסט מחולץ. הוא מזהה את זהות בית העסק בשתי דרכים משלימות:

1. **הלוגו** — הסמל/מותג המודפס בראש הקבלה (רשת מזון, תחנת דלק, מלון, חברת מוניות וכו׳). ה-prompt מנחה לזהות את בית העסק מ״הטקסט הגדול ביותר בראש הקבלה״, ובמסופי סליקה — את **שם העסק ולא את מותג המסוף** (`OracleAiService.cs:600` — `VENDOR: Largest text at TOP. For terminals: business name, NOT terminal brand`).
2. **שם העסק** — שם ה-merchant שמחולץ ל-`merchant.name` (`OracleAiService.cs:630`).

מזהות בית העסק הזו ה-AI **מסיק את סוג ההוצאה**. דוגמאות:

| מה שרואים בלוגו/שם העסק | Expense Type שמזוהה |
|---------------------------|----------------------|
| מסעדה / בית קפה / רשת מזון | `business_meal` / `meal` |
| תחנת דלק / חניון | `vehicle` / `parking` |
| מלון / בית הארחה | `hotel` |
| חברת מוניות / Gett / Uber | `taxi` |
| ספק אינטרנט / תקשורת | `internet` |
| בית עסק לבידור | `entertainment` |
| כשאין זיהוי חד-משמעי | `other` |

כלומר: **הלוגו ושם העסק הם אות הזיהוי המרכזי** לקביעת ה-Expense Type — לפניהם וביחד עם הפריטים הנרכשים.
